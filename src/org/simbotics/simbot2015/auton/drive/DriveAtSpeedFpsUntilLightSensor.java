...
public class DriveAtSpeedFpsUntilLightSensor extends AutonCommand {
	...
	public DriveAtSpeedFpsUntilLightSensor(double speedFps,double heading,double maxOutput,boolean AccelerationControl,long timeout,boolean goFaster) {
	...
	public boolean calculate() {
		...
		if(this.AccelerationControl){
			if(this.goFaster){
				this.output= this.speedCap.calculate(this.maxOutput,true);
			}else{
				this.output= this.speedCap.calculate(this.maxOutput);
			}
		}else{
        ...
        if(this.hasToteCount > 0) { // we see a tote
            ...
            // not done yet
        	this.driveControl.gyroAtSpeedFps(this.speedFps,this.output);
